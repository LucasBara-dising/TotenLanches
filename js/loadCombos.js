    ...
    fetch('./json/combosRepouse.json').then((response) => {
        response.json().then((dados) => {
          ...
          for(i=0; i<dados.combo_Hamburguer.length; i++){
            criaCard(dados.combo_Hamburguer, i, dados.combo_Veg[i].valor, "conteinerHambuerger", "Hambuerger", "combo")
          }

          for(i=0; i<dados.combo_Salsicha.length; i++){
            criaCard(dados.combo_Salsicha, i, dados.combo_Veg[i].valor, "conteinerSalsicha", "Salsicha", "combo")
          }
